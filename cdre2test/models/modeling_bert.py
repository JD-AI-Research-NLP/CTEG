...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            attention_scores = attention_scores * gate
...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        self_outputs = self.self(
            hidden_states, attention_mask, head_mask,gate, encoder_hidden_states, encoder_attention_mask
        )
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        self_attention_outputs = self.attention(hidden_states, attention_mask, head_mask,gate)
        attention_output = self_attention_outputs[0]
        outputs = self_attention_outputs[1:]  # add self attentions if we output attention weights

        if self.is_decoder and encoder_hidden_states is not None:
            cross_attention_outputs = self.crossattention(
                attention_output, attention_mask, head_mask, gate, encoder_hidden_states, encoder_attention_mask
            )
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs = layer_module(
                hidden_states, attention_mask, head_mask[i],gate, encoder_hidden_states, encoder_attention_mask
            )
